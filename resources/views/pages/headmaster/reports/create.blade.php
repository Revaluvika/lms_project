...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-100 bg-slate-50/50">
            <h1 class="text-xl font-bold text-slate-800">Buat Laporan Baru</h1>
            <p class="text-sm text-slate-500 mt-1">Isi formulir berikut untuk mengirimkan laporan ke Dinas Pendidikan.</p>
        </div>

        <form action="{{ route('school-reports.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition placeholder:text-slate-400" 
                               placeholder="Contoh: Laporan Keaktifan Siswa Bulan Agustus 2024" 
                    ...
                    <div>
                        ...
                        <select name="report_type" id="report_type" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition" required>
                            ...
                            <option value="Bulanan" {{ old('report_type') == 'Bulanan' ? 'selected' : '' }}>Laporan Bulanan</option>
                            <option value="Semester" {{ old('report_type') == 'Semester' ? 'selected' : '' }}>Laporan Semester</option>
                            <option value="Tahunan" {{ old('report_type') == 'Tahunan' ? 'selected' : '' }}>Laporan Tahunan</option>
                            <option value="Keuangan" {{ old('report_type') == 'Keuangan' ? 'selected' : '' }}>Laporan Keuangan</option>
                            <option value="Insidental" {{ old('report_type') == 'Insidental' ? 'selected' : '' }}>Laporan Insidental</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="report_period" id="report_period" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition" 
                               value="{{ old('report_period') }}" required>
                        <p class="text-xs text-slate-500 mt-2 flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            Tanggal referensi laporan
                        </p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition placeholder:text-slate-400" 
                              placeholder="Tuliskan ringkasan isi laporan atau catatan penting untuk Dinas...">{{ old('description') }}</textarea>
                </div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-slate-300 rounded-xl hover:bg-slate-50 hover:border-indigo-400 transition-colors group cursor-pointer relative">
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-slate-600 justify-center">
                                <span class="font-medium text-indigo-600 hover:text-indigo-500">Upload sebuah file</span>
                                <p class="pl-1">atau drag and drop</p>
                            </div>
                            <p class="text-xs text-slate-500">
                                PDF, DOC, DOCX, XLS, XLSX hingga 10MB
                            </p>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-slate-100">
                <a href="{{ route('school-reports.index') }}" class="px-6 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-colors">
                    Batal
                </a>

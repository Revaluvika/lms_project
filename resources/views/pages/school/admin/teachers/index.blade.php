...
<div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Kelola data guru, mata pelajaran, dan import massal.</p>
        </div>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="font-mono text-gray-600">{{ $teacher->nip }}</span>
                                <span class="text-xs text-gray-400">{{ $teacher->user->email ?? '-' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">{{ $teacher->specialization ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $teacher->telepon ?? '-' }}</td>
                        <td class="px-6 py-4 text-right flex justify-end gap-2">
                            <button onclick="openResetPasswordModal('{{ route('master.teachers.reset-password', $teacher->id) }}')" class="p-1.5 hover:bg-yellow-50 rounded-lg text-gray-400 hover:text-yellow-600 transition-colors" title="Reset Password">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                            </button>
                            <button onclick="openEditModal({{ $teacher->id }}, '{{ $teacher->nama }}', '{{ $teacher->user->email ?? '' }}', '{{ $teacher->nip }}', '{{ $teacher->specialization }}', '{{ $teacher->telepon }}', '{{ $teacher->alamat }}')" class="p-1.5 hover:bg-blue-50 rounded-lg text-gray-400 hover:text-blue-600 transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button onclick="openDeleteModal('{{ route('master.teachers.destroy', $teacher->id) }}')" class="p-1.5 hover:bg-red-50 rounded-lg text-gray-400 hover:text-red-600 transition-colors" title="Hapus">
...
<x-modal name="import-modal" :show="false" focusable>
    <div class="p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-bold text-lg text-gray-800">Import Data Guru</h3>
        ...
        <form action="{{ route('master.teachers.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="text-right">
                <a href="{{ route('master.teachers.template') }}" class="text-blue-600 text-sm hover:underline">Download Template Excel</a>
            </div>
...
<x-modal name="create-modal" :show="false" focusable>
    <div class="p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-bold text-lg text-gray-800">Tambah Guru Manual</h3>
            <button x-on:click="$dispatch('close-modal', 'create-modal')" class="text-gray-400 hover:text-gray-600">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form action="{{ route('master.teachers.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nip" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" name="specialization" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
            </div>
            <div>
...
<x-modal name="edit-modal" :show="false" focusable>
    <div class="p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-bold text-lg text-gray-800">Edit Data Guru</h3>
        ...
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">NIP</label>
                    <input type="text" id="editNip" name="nip" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
                <input type="text" id="editMapel" name="specialization" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Telepon</label>
...
<x-modal name="delete-modal" :show="false" focusable>
    <div class="p-6">
        ...
        <div class="mt-3 text-center sm:mt-5">
            <h3 class="text-lg font-medium leading-6 text-gray-900">Hapus Data Guru?</h3>
            <div class="mt-2">
                <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus data guru ini? Akun pengguna terkait juga akan dihapus. Tindakan ini tidak dapat dibatalkan.</p>
            </div>
        </div>
...
<script>
    function openEditModal(id, nama, email, nip, mapel, telepon, alamat) {
        document.getElementById('editForm').action = `/master/teachers/${id}`;
